...
{
    public class HPLarge : HPAbstract
    {
        public HPLarge(IWebDriver driver, Viewport viewport, Language language) : base(driver, viewport, language) { }

        #region SSO
        public override void LaunchSsoModal()
        {
            ClickElement(FindElement(By.CssSelector(HeaderFooterLocators.Menus.MyAccountToggleBtn.GetAttribute<DescriptionAttribute>().Description)));
            WaitForSubMenuDropdownStatus();
            ClickElement(FindElement(By.CssSelector(HeaderFooterLocators.Menus.MyAccountToggleBtn.GetAttribute<DescriptionSubMenuSignInAttribute>().DescriptionSubMenuSignIn)));
        ...
        public override void WaitForSubMenuDropdownStatus(bool isExpanded = true)
        {
            if (!isExpanded)
            {
                WaitUntil(() => !FindElement(By.CssSelector(HeaderFooterLocators.Menus.MyAccountToggleState.GetAttribute<DescriptionAttribute>().Description)).GetAttribute("class").Contains("open"));
            }
            else
            {
                WaitUntil(() => FindElement(By.CssSelector(HeaderFooterLocators.Menus.MyAccountToggleState.GetAttribute<DescriptionAttribute>().Description)).GetAttribute("class").Contains("open"));
        ...
        public override void LoginToSsoAccount(SsoLogin.SsoAccountType ssoAccountType, string email, string password)
        {
            switch (ssoAccountType)
            {
                ...
                case SsoLogin.SsoAccountType.SocialAccountFB:
                    ...
                    EnterText(By.CssSelector(MyGarageLocators.Login.FbUserName.GetAttribute<DescriptionAttribute>().Description), email);
                    EnterText(By.CssSelector(MyGarageLocators.Login.FbPassword.GetAttribute<DescriptionAttribute>().Description), password);
                    ClickElement(FindElement(By.CssSelector(MyGarageLocators.Login.FbLogin.GetAttribute<DescriptionAttribute>().Description)));
        ...
        public override void LogoutFromSsoAccount()
        {
            ClickElement(FindElement(By.CssSelector(HeaderFooterLocators.Menus.MyAccountToggleBtn.GetAttribute<DescriptionAttribute>().Description)));
            WaitForSubMenuDropdownStatus();
            ClickElement(FindElement(By.CssSelector(HeaderFooterLocators.Menus.SignOutSubMenuBtn.GetAttribute<DescriptionAttribute>().Description)));
